...
def main(
    ...
    if not webcam:
        file_path = increment_path(
            save_dir / f"{Path(video_source).stem}.mp4", exist_ok=exist_ok
        )
    else:
    ...
    while capture.isOpened():
        ...
        if results[0].boxes.id is not None:
            ...
            for box, track_id, cls in zip(boxes, track_ids, clss):
                ...
                cv2.polylines(frame, [points], False, colors(cls, True), 2)
        ...
        for region in counting_regions:
            ...
            text_size, _ = cv2.getTextSize(region_label, cv2.FONT_HERSHEY_SIMPLEX, 1, 2)
            ...
            cv2.polylines(frame, [polygon_coords], True, region_color, 2)
